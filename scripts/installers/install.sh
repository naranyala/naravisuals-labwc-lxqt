...
if ! step_done "$step_name"; then
    ...
    if ! step_done "$step_name"; then
        ...
        if bash "$SCRIPT_DIR/build/install.sh" $INSTALLER_ARGS 2>>"$LOG_FILE"; then
            log_ok "Dotfiles installed"
            step_mark "$step_name"
        else
...
# ---- Step 4: Build LXQt from source (optional, full mode) ----
step_name="build-lxqt"
if [ "$MODE" = "full" ] && [ -d "$SCRIPT_DIR/lxqt" ]; then
    if ! step_done "$step_name"; then
        log_step "Building LXQt components from source"
        log_info "This builds the essential LXQt libraries and components"
        log_info "from the lxqt/ source tree (~244 MB of sources)."
        log_info "Skip this step if you use distro packages: sudo apt install lxqt-session lxqt-panel"
        if [ "$DRY_RUN" = true ]; then
            log_dim "[DRY-RUN] Would run: bash scripts/installers/build-lxqt-from-source.sh --essential"
        else
            if bash "$SCRIPT_DIR/build-lxqt-from-source.sh" --essential 2>>"$LOG_FILE"; then
                log_ok "LXQt components built from source"
                step_mark "$step_name"
            else
                log_warn "LXQt source build had issues (check log)"
                log_info "You can retry: bash scripts/installers/build-lxqt-from-source.sh --full"
                log_info "Or use distro packages: sudo apt install lxqt-session lxqt-panel pcmanfm-qt qterminal"
            fi
        fi
    fi
fi
...
if [ "$MODE" = "minimal" ]; then
    ...
    if ! step_done "$step_name"; then
        ...
        if [ "$DRY_RUN" = true ]; then
            ...
            if bash "$SCRIPT_DIR/install-all.sh" $select_flag $run_flags 2>>"$LOG_FILE"; then
...
if [ -f "$VALIDATE_SCRIPT" ]; then
    if bash "$VALIDATE_SCRIPT" --quiet 2>/dev/null; then
        log_ok "Validation passed"
    else
        log_warn "Some validation checks failed"
        log_info "Run: bash scripts/utils/validate.sh"
    fi
fi
...
step_done "build-lxqt"          && printf "  [done] LXQt from source\n"         || printf "  [--] LXQt from source\n"
...
printf "  2. Run: ${GREEN}bash scripts/installers/install-all.sh --select${RST} for additional themes\n"
printf "\n${BOLD}Troubleshooting:${RST}\n"
printf "  - View log: ${GREEN}cat %s${RST}\n" "$INSTALL_LOG"
printf "  - Validate: ${GREEN}bash scripts/utils/validate.sh${RST}\n"
printf "  - Restore:  ${GREEN}bash scripts/installers/build/install.sh --restore${RST}\n"
