...
check_autostart_entry "$AUTOSTART" "cliphist" "Clipboard manager"
check_autostart_entry "$AUTOSTART" "flameshot" "Screenshot daemon"
check_autostart_entry "$AUTOSTART" "redshift" "Night light"
...
if [ "$JSON_MODE" = false ]; then
    step "Checking config drift against source dotfiles"
fi

DRIFT_DIR="$SCRIPT_DIR/configs/dotfiles"
DRIFT_MAP="labwc/rc.xml|config/labwc/rc.xml
labwc/autostart|config/labwc/autostart
labwc/environment|config/labwc/environment
lxqt/session.conf|config/lxqt/session.conf
lxqt/panel.conf|config/lxqt/panel.conf
lxqt/lxqt.conf|config/lxqt/lxqt.conf
...
checksum_cmd=""
for c in md5sum sha256sum cksum; do
    command -v "$c" &>/dev/null && checksum_cmd="$c" && break
done

cksum_file() {
    local f="$1"
    [ -n "$checksum_cmd" ] || { stat -c%s "$f" 2>/dev/null || echo 0; return; }
    case "$checksum_cmd" in
        md5sum|sha256sum) "$checksum_cmd" "$f" | awk '{print $1}' ;;
        cksum) "$checksum_cmd" "$f" | awk '{print $1}' ;;
    esac
}

while IFS='|' read -r src_rel dst_rel; do
    [ -z "$src_rel" ] && continue
    src_path="$DRIFT_DIR/$src_rel"
    dst_path="$HOME_DIR/$dst_rel"

    if [ ! -f "$src_path" ]; then
        DRIFT_MISS=$((DRIFT_MISS + 1))
        continue
    fi
    if [ ! -f "$dst_path" ]; then
        warn "Config drift: $dst_rel not deployed" "Run: bash install.sh"
        DRIFT_MOD=$((DRIFT_MOD + 1))
        continue
    fi

    src_cksum=$(cksum_file "$src_path")
    dst_cksum=$(cksum_file "$dst_path")
    if [ "$src_cksum" != "$dst_cksum" ]; then
        warn "Config drift: $dst_rel differs from source" "Re-deploy: bash install.sh --force"
        DRIFT_MOD=$((DRIFT_MOD + 1))
    else
        DRIFT_OK=$((DRIFT_OK + 1))
    fi
...
if [ "$DRIFT_OK" -gt 0 ]; then
    pass "$DRIFT_OK deployed configs match source"
fi
